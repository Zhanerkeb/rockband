...
class Articles extends Component {
    ...
    getData = e => {
        axios.get(`${base_url}/articles/get.php`).then(res => {
            this.setState({articles: res.data});
        })
    }

    handleDelete = id => {
        axios.get(`${base_url}/articles/delete.php?id=${id}`).then(res => {
            this.getData();
        }).catch(err => {
            alert(err);
        })
    ...
    handleOk = e => {
        if (this.state.title.length > 0) {
            ...
            axios.post(`${base_url}/articles/save.php`, fm, {
                headers: {
    ...
    handleEditOk = e => {
        ...
        if (this.state.edittitle.length > 0) {
            ...
            axios.post(`${base_url}/articles/update.php`, fm,{
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            }).then(res => {
    ...
    <Divider type="vertical"/>
        ...
        return (
            <div>
                <Button type="primary" onClick={this.showModal} className={'mb-20'}>Добавить статью</Button>
                <Modal
                    visible={visible}
